...
public class Triangle extends Polygon{
    ...
    protected List<GeoPoint> findGeoIntersectionsHelper(Ray ray) {
        Point p0 = ray.getP0(); // get point
        Vector v = ray.getDir(); // get direction
        List<Intersectable.GeoPoint> trianglePlane = plane.findGeoIntersections(ray);
        if(trianglePlane == null){ // if there is no intersection between plane and ray then of course there won't be an intersection between ray and triangle
            return null;
        }
        ...
        if(allNegative || allPositive){
            return List.of(new GeoPoint(this, trianglePlane.get(0).point)); // return the intersections with the plane that the triangle is on
        }
        return null; // else
    }
@Override
